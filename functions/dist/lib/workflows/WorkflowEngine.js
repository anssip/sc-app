export class WorkflowEngine {
    ...
    static analyzeIntent(message, _chartContext) {
        const lowerMessage = message.toLowerCase();
        // Check for trend line related keywords
        if (lowerMessage.includes("trend line") ||
    ...
    static async executeWorkflow(workflowName, input) {
    ...
    static convertToToolCalls(result, workflowType) {
        const toolCalls = [];
        switch (workflowType) {
    ...
    static getConfirmationMessage(workflowType, result) {
        switch (workflowType) {
            case "TrendLineWorkflow":
                return `Found ${result.points.length} collinear points with ${(result.confidence * 100).toFixed(1)}% confidence. ${result.type} line added to chart.`;
            case "PatternRecognitionWorkflow":
                const patterns = result.patterns.map((p) => p.type).join(", ");
                return `Identified patterns: ${patterns}. Highlighted on chart.`;
            case "BacktestingWorkflow":
                return `Backtesting complete: ${result.performance.totalTrades} trades, ${(result.performance.winRate * 100).toFixed(1)}% win rate, ${(result.performance.totalReturn * 100).toFixed(2)}% return.`;
            default:
