...
const getOpenAI = () => {
  ...
  // Only use OPENAI_API_KEY from environment (dotenv)
...
export async function processChat({
  ...
  try {
    ...
    const stream = await client.chat.completions.create({
      ...
      stream: true,
      temperature: 0.7,
      // Updated to use correct indicator IDs from Firestore
      max_tokens: 1000,
    });
      ...
      if (toolCall.function.name && toolCall.function.arguments) {
        try {
          ...
          if (chartTools.isChartTool(toolCall.function.name)) {
            ...
            // Price data tools now use REST API, others still use Firestore
