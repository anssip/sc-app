...
export class MarketAPI {
    API_BASE_URL = "https://market.spotcanvas.com";
     ...
     */
    async fetchPriceData(symbol, interval, startTime, endTime, onProgress, evaluators) {
        try {
            ...
            console.log(`  Time difference: ${timeDiff}ms (${daysDiff.toFixed(2)} days)`);
            if (startTime >= endTime) {
                console.error("ERROR: startTime >= endTime!");
                throw new Error("Invalid time range: start time must be before end time");
            }
            const intervalMs = this.getIntervalMilliseconds(interval);
            console.log(`  intervalMs: ${intervalMs} (${intervalMs / 1000 / 60} minutes)`);
            // Align timestamps to interval boundaries and ensure they're integers
            const alignedStart = Math.floor(Math.floor(startTime / intervalMs) * intervalMs);
            const alignedEnd = Math.floor(Math.ceil(endTime / intervalMs) * intervalMs);
            ...
            console.log(`  Math.floor(${startTime / intervalMs}) = ${Math.floor(startTime / intervalMs)}`);
            console.log(`  endTime / intervalMs = ${endTime / intervalMs}`);
            console.log(`  Math.ceil(${endTime / intervalMs}) = ${Math.ceil(endTime / intervalMs)}`);
            console.log(`  alignedStart: ${alignedStart} (${new Date(alignedStart).toISOString()})`);
            console.log(`  alignedEnd: ${alignedEnd} (${new Date(alignedEnd).toISOString()})`);
            console.log(`  Aligned diff: ${alignedEnd - alignedStart}ms`);
            // Calculate total candles needed
            const totalCandles = Math.floor((alignedEnd - alignedStart) / intervalMs) + 1;
            console.log(`  totalCandles calculation: Math.floor((${alignedEnd} - ${alignedStart}) / ${intervalMs}) + 1 = ${totalCandles}`);
            console.log(`  totalCandles needed: ${totalCandles}`);
            console.log(`  Expected candles for ${daysDiff.toFixed(1)} days at ${interval}: ~${Math.floor(timeDiff / intervalMs)}`);
            if (totalCandles <= this.MAX_CANDLES_PER_REQUEST) {
                // Single request is sufficient
                return await this.fetchSingleBatch(symbol, interval, alignedStart, alignedEnd, evaluators);
            }
            ...
            log(`• Fetching in batches of ${this.MAX_CANDLES_PER_REQUEST} candles...`);
            const results = [];
            let currentStart = alignedStart;
            let batchNumber = 1;
            const totalBatches = Math.ceil(totalCandles / this.MAX_CANDLES_PER_REQUEST);
            while (currentStart < alignedEnd) {
                ...
                const batchCandles = Math.min(remainingCandles, this.MAX_CANDLES_PER_REQUEST);
                const batchEnd = Math.min(currentStart + (batchCandles - 1) * intervalMs, alignedEnd);
                log(`• Fetching batch ${batchNumber}/${totalBatches} (${new Date(currentStart).toLocaleDateString()} to ${new Date(batchEnd).toLocaleDateString()})`);
                const batchData = await this.fetchSingleBatch(symbol, interval, currentStart, batchEnd, evaluators);
    ...
    async fetchSingleBatch(symbol, interval, startTime, endTime, evaluators) {
        ...
        console.log(`Fetching price data from API: ${this.API_BASE_URL}/history?${params}`);
        console.log(`• Symbol: ${symbol}`);
        console.log(`• Granularity: ${interval}`);
        console.log(`• StartTime: ${startTime} (${new Date(startTime).toISOString()})`);
        console.log(`• EndTime: ${endTime} (${new Date(endTime).toISOString()})`);
        console.log(`• Time range: ${(endTime - startTime) / (1000 * 60 * 60 * 24)} days`);
        console.log(`• Expected candles: ~${Math.floor((endTime - startTime) / this.getIntervalMilliseconds(interval))}`);
        console.log(`• Full URL: ${this.API_BASE_URL}/history?${params.toString()}`);
        ...
        if (result.candles && result.candles.length > 0) {
            ...
            console.log(`• First candle: ${new Date(Number(firstCandle.timestamp)).toISOString()}`);
            console.log(`• Last candle: ${new Date(Number(lastCandle.timestamp)).toISOString()}`);
        }
